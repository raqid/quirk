...
export default function CaptureScreen({ navigation }) {
  ...
  const activeTasks = mockTasks.filter((t) => t.data_type === selectedType).slice(0, 3);
  ...
  const handleAudio = () => {
    navigation.navigate('UploadMetadata', { asset: { isMock: true }, type: 'audio', taskId: linkedTask });
  };
            ...
            {activeTasks.map((task) => (
              ...
              >
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.taskPay}>+${task.pay_per_submission.toFixed(2)} per submission</Text>
                </View>
                {linkedTask === task.id && <Text style={styles.taskCheck}>✓</Text>}
              </TouchableOpacity>
            ))}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  taskRowActive: { borderColor: colors.primary },
  ...
  taskCheck: { color: colors.primary, fontWeight: '700', fontSize: 16 },
